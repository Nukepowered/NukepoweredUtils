...
public class GeneratorFuels {
	public static void init() {
      ...
      //Semifuels
        FuelRecipes.registerSemiFluidGeneratorFuel(NPUMaterials.FishOil.getFluid(64), 1, GTValues.LV);
        if (Loader.isModLoaded("forestry") && NPUConfig.integration.ForestryIntegration)
	...
	public static void registerNaquadahReactorFuel(FluidStack fuelStack, int duration, int tier) {
		NPURecipeMaps.NAQUADAH_REACTOR_FUELS.addRecipe(new FuelRecipe(fuelStack, duration, GTValues.V[tier]));
	}
	
	private static void removeAllRecipes(FuelRecipeMap map) {
		List<FuelRecipe> recipe = new ArrayList<>();
		recipe.addAll(map.getRecipeList());
		
		for (FuelRecipe r : recipe) {
			map.removeRecipe(r);
		}
	}
}
